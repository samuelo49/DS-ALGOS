"""

❓ PROMPT
Euclid's algorithm for finding the greatest common divisor (GCD) of two numbers *a* and *b* is:

euclidGCD(a, 0) := a
euclidGCD(a, b) := gcd(b, a mod b)

Write a function that implements Euclid's algorithm and finds the GCD of two numbers.

Check out this [video](https://www.youtube.com/watch?v=Jwf6ncRmhPg) for more information on this very old algorithm. [Wikipedia](https://en.wikipedia.org/wiki/Euclidean_algorithm) also has a formal write-up.

Example(s)
euclidGCD(9, 27) == 9
euclidGCD(27, 9) == 9
euclidGCD(3, 1) == 1


🔎 EXPLORE
State your assumptions & discoveries:


Create examples & test cases:


🧠 BRAINSTORM
What approaches could work? Consider data structures or algorithmic patterns.
Analyze the space & time complexity.
Approach 1:
Time: O()
Space: O()


📆 PLAN
High-level outline of approach #:


🛠️ IMPLEMENT
function euclidGCD(a, b) {
def euclidGCD(a: int, b: int) -> int:


🧪 VERIFY
Run your examples & test cases.
Methodically analyze and debug issue(s).

"""


def euclidGCD(a: int, b: int) -> int:
    if b == 0:
        return a
    else:
        return euclidGCD(b, a % b)
