...
def search(nums, target):
    ...
    while leftPtr <= rightPtr:
        ...
        elif nums[mid] >= nums[leftPtr]:
            if target >= nums[leftPtr] and target < nums[mid]:
                #continue to search left side
                rightPtr = mid - 1
            else:
                #continue to search right side
                leftPtr = mid + 1
        else:   # right side is sorted
            if target < nums[rightPtr] and target >= nums[mid]:
                #continue to search right side
                leftPtr = mid + 1
            else:
                #continue to search left side
